...
def weather(data,event,q):#(dp,user_id):
    ...
    completion = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=[{"role": "user", "content": prompt}], max_length=850)["choices"][0]["message"]["content"]
...
def facts(data,event,q):
    ...
    completion = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=[{"role": "user", "content": prompt}], max_length=750)["choices"][0]["message"]["content"]
    # await dp.storage.update_data(user_id,weather=completion)
    print("я всё x3")
    j = completion.find('1')
    completion = completion[j:]
...
def places(data,event,q):
    ...
    # data = await dp.storage.get_data(user_id)
    print(data)
    prompt = "Расскажи мне про 5 самых интересных мест {} с очень кратким описанием.  Напиши не более 750 символов в формате: 1. Первое место 2. Второе место, и так далее".format(data["GEO"]["city"])
    completion = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=[{"role": "user", "content": prompt}], max_length=750)["choices"][0]["message"]["content"]
    # await dp.storage.update_data(user_id,weather=completion)
    print("я всё x4")
    j = completion.find('1')
